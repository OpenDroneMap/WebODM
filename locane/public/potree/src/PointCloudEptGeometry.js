...
export class PointCloudEptGeometry extends BaseGeometry {
	...
	constructor(base, ept,jwt) {
		...
		this.loader = (() => {
			switch (ept.dataType) {
				case 'laszip': return new Potree.EptLaszipLoader(this.jwt)
	...
	async loadHierarchyPage(key) {
		...
		console.log(filename+"wfcevufhvbweudvchfihgf")
		const response = await fetch(filename,{headers:{'Authorization':'JWT '+this.jwt}});
