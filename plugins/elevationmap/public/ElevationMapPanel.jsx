...
export default class ElevationMapPanel extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      reference: "Sea",
      noiseFilterSize:
        Storage.getItem("last_elevationmap_noise_filter_size") || "3",
      customNoiseFilterSize:
        Storage.getItem("last_elevationmap_custom_noise_filter_size") || "3",
  ...
  componentDidUpdate() {
    if (this.props.isShowed && this.state.loading) {
      ...
      this.loadingReq = $.getJSON(`/api/projects/${project}/tasks/${id}/`)
        ...
        .fail(() => {
          this.setState({
            permanentError: `Cannot retrieve information for task ${id}. Are you are connected to the internet?`,
          });
        })
        .always(() => {
  ...
  getFormValues = () => {
    ...
    return {
      ...
      noise_filter_size:
        noiseFilterSize !== "custom" ? noiseFilterSize : customNoiseFilterSize,
      reference,
    };
  };
    ...
    $.getJSON(url)
      .done((geojson) => {
        try {
          ...
          const allLevels = geojson.features
            .map((feature) => [
          ...
          geojson.features.forEach((levelFeature) => {
            ...
            const rgbHex = this.heatmap_coloring(
              (bottom + top) / 2,
              lowestLevel,
              highestLevel
            );
            const areaInLevel = area(levelFeature).toFixed(2);
            let geojsonForLevel = L.geoJSON(levelFeature)
              .setStyle({
                color: rgbHex,
                fill: true,
                fillColor: rgbHex,
              ...
              .bindPopup(
                `Altitude: Between ${bottom}m and ${top}m<BR>Area: ${areaInLevel}m2`
              )
              .on("popupopen", (popup) => {
                ...
                featureGroup
                  .getLayers()
                  .forEach((layer) =>
                    layer.setStyle({ fillOpacity: 0.4 * this.state.opacity })
                  );
                popup.propagatedFrom
                  .setStyle({ color: "black", fillOpacity: this.state.opacity })
                  .bringToFront();
              })
              .on("popupclose", (popup) => {
                ...
                featureGroup
                  ...
                  .forEach((layer) =>
                    layer
                      .bringToFront()
                      .setStyle({ fillOpacity: this.state.opacity })
                  );
  ...
  generateElevationMap = (data, loadingProp, isPreview) => {
    ...
    Storage.setItem(
      "last_elevationmap_custom_interval",
      this.state.customInterval
    );
    Storage.setItem(
      "last_elevationmap_noise_filter_size",
      this.state.noiseFilterSize
    );
    Storage.setItem(
      "last_elevationmap_custom_noise_filter_size",
      this.state.customNoiseFilterSize
    );
  ...
  handleChangeOpacity = (evt) => {
    ...
    this.state.previewLayer.setStyle({
      opacity: opacity,
      fillOpacity: opacity,
    });
  ...
  render() {
    ...
    const noiseFilterSizeValues = [
      { label: "Do not filter noise", value: 0 },
        ...
        <div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              ...
              <p
                ...
                data-tip="You have two options:<br/>&#8226; Insert your custom elevation intervals, in the form: 10-15,20-30. <br/>&#8226; Insert a number (for example 5) and the intervals will be auto generated every 5 meters based on the elevation data."
              />
            </div>
          </div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              ...
              <p
                className="fas fa-info-circle help"
                data-tip="You can determine if the intervals specified above will be based on the sea level, or on the ground.<br/>Take into account that in order to be able to select 'ground' you need to have run the task with the --dtm option."
              />
            </div>
          </div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              <select
                className="form-control"
                value={noiseFilterSize}
                onChange={this.handleSelectNoiseFilterSize}
              >
                {noiseFilterSizeValues.map((sv) => (
                  <option value={sv.value}>
                    {sv.label} ({sv.value} meter)
                  </option>
                ))}
              ...
              <p
                className="fas fa-info-circle help"
                data-tip="You can determine the diameter of the area used to filter noise."
              />
            </div>
          </div>
            ...
            <div className="row form-group form-inline">
              ...
              <div className="col-sm-9 ">
                <input
                  ...
                  value={customNoiseFilterSize}
                  onChange={this.handleChangeCustomNoiseFilterSize}
                />
                <span> meter</span>
              </div>
            </div>
          ...
          {previewLayer ? (
            <div className="row form-group form-inline">
              ...
              <div className="col-sm-9">
                ...
                <p
                  className="fas fa-info-circle help"
                  data-tip="Control the opacity of the elevation map. You must generate a preview to be able to control the opacity."
                />
    ...
    return (
      <div className="elevationmap-panel">
