...
export default class ChangeDetectionPanel extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      displayType:
        Storage.getItem("last_changedetection_display_type") || "contours",
      ...
      align: this.props.alignSupported
        ? Storage.getItem("last_changedetection_align") === "true"
        : false,
      other: "",
  ...
  componentDidUpdate() {
    if (this.props.isShowed && this.state.loading) {
      ...
      this.loadingReq = $.getJSON(`/api/projects/${project}/tasks/`)
        .done((res) => {
          ...
          if (!this.props.alignSupported) {
            ...
            if (myAssets.indexOf("dsm.tif") === -1)
              errors.push(
                "No DSM is available. Make sure to process a task with either the --dsm option checked"
              );
            if (myAssets.indexOf("dtm.tif") === -1)
              errors.push(
                "No DTM is available. Make sure to process a task with either the --dtm option checked"
              );
            ...
            const otherTasksWithDEMs = res
              .filter(({ id }) => id !== taskId)
              .filter(
                ({ available_assets }) =>
                  available_assets.indexOf("dsm.tif") >= 0 &&
                  available_assets.indexOf("dtm.tif") >= 0
              );

            if (otherTasksWithDEMs.length === 0) {
              this.setState({
                permanentError:
                  "Couldn't find other tasks on the project. Please make sure there are other tasks on the project that have both a DTM and DSM.",
              });
              return;
            }
            otherTasksWithDEMs.forEach(({ id, name }) =>
              otherTasksInProject.set(id, name)
            );
          } else {
            res
              .filter(({ id }) => id !== taskId)
          ...
          if (otherTasksInProject.size === 0) {
            this.setState({
              permanentError: `Couldn't find other tasks on this project. This plugin must be used on projects with 2 or more tasks.`,
            });
          } else {
            const firstOtherTask = Array.from(
              otherTasksInProject.entries()
            )[0][0];
        ...
        .fail(() => {
          this.setState({
            permanentError: `Cannot retrieve information for the current project. Are you are connected to the internet?`,
          });
        })
        .always(() => {
    ...
    $.getJSON(url)
      .done((result) => {
        try {
          ...
          if (result.max) {
            const heatMap = L.heatLayer(result.values, {
              max: result.max,
              radius: 9,
              minOpacity: 0,
            });
            heatMap.setStyle = ({ opacity }) =>
              heatMap.setOptions({ max: result.max / opacity });
            ...
            result.features.forEach((feature) => {
              ...
              let geojsonForLevel = L.geoJSON(feature).bindPopup(
                `Area: ${area}m2<BR/>Min: ${min}m<BR/>Max: ${max}m<BR/>Avg: ${avg}m<BR/>Std: ${std}m`
              );
  ...
  generateChangeMap = (data, loadingProp, isPreview) => {
    ...
    Storage.setItem(
      "last_changedetection_display_type",
        ...
        <div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              ...
              <p
                className="fas fa-info-circle help"
                data-tip="This plugin will take the reference task, and substract the compare task. Then, we will apply the filters<BR/>available below to determine if some difference is a valid change or not."
              />
            </div>
          </div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              <select
                className="form-control"
                value={other}
                onChange={this.handleSelectOther}
              >
                {Array.from(otherTasksInProject.entries()).map(([id, name]) => (
              ...
              {this.props.alignSupported ? (
                <p
                  className="fas fa-info-circle help"
                  data-tip="Select the other task on the project to compare this task against."
                />
              ) : (
                <p
                  className="fas fa-info-circle help"
                  data-tip="Select the other task on the project to compare this task against.<BR/>Take into account that only tasks with both a DSM and DTM will be available here."
                />
              )}
            </div>
          </div>
          ...
          {this.props.alignSupported ? (
            <div className="row form-group form-inline">
              ...
              <div className="col-sm-9 ">
                ...
                <p
                  className="fas fa-info-circle help"
                  data-tip="It is possible to align the two tasks to detect changes more accurately.<BR/>But take into account that the processing can take longer if you do so."
                />
              </div>
            </div>
          ) : (
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              <select
                className="form-control"
                value={displayType}
                onChange={this.handleSelectDisplayType}
              >
              ...
              <p
                className="fas fa-info-circle help"
                data-tip="You can select to display a heatmap with all the substraction, or the contours of the filtered changes.<BR/>Export is only available for the 'Contours' mode."
              />
            </div>
          </div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              <input
                type="number"
                className="form-control custom-interval"
                value={resolution}
                onChange={this.handleSelectResolution}
              />
              <span> meters/pixel</span>
              <p
                className="fas fa-info-circle help"
                data-tip="You can indicate the resolution to use when detecting changes. The final resolution used will be: max(input, resolution(reference), resolution(compare)).<BR/>The higher the resolution, the faster the result will be calculated. You can set to 0 to use the DEMs resolutions."
              />
            </div>
          </div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              <input
                ...
                className="form-control custom-interval"
                value={minHeight}
                onChange={this.handleSelectMinHeight}
              />
              <span> meters</span>
              <p
                className="fas fa-info-circle help"
                data-tip="When detecting change, there can be some noise. Please indicate the min height that change needs to have to consider it a valid change."
              />
            </div>
          </div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              <input
                type="number"
                disabled={displayType === "heatmap"}
                className="form-control custom-interval"
                value={minArea}
                onChange={this.handleSelectMinArea}
              />
              <span> sq meters</span>
              <p
                className="fas fa-info-circle help"
                data-tip="When detecting change, there can be some noise. Please indicate the min area that change needs to have to consider it a valid change.<BR/>This option is only available with the 'Contours' display mode."
              />
            </div>
          </div>
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              <select
                className="form-control"
                value={epsg}
                onChange={this.handleSelectEpsg}
              >
            ...
            <div className="row form-group form-inline">
              ...
              <div className="col-sm-9 ">
                <input
                  ...
                  className="form-control custom-interval"
                  value={customEpsg}
                  onChange={this.handleChangeCustomEpsg}
                />
              </div>
            </div>
          ) : (
          ...
          {previewLayer ? (
            <div className="row form-group form-inline">
              ...
              <div className="col-sm-9">
                ...
                <p
                  className="fas fa-info-circle help"
                  data-tip="Control the opacity of the change map. You must generate a preview to be able to control the opacity."
                />
          ...
          <div className="row action-buttons">
            <div className="col-sm-9 text-right">
              <button
                ...
                type="button"
                className="btn btn-sm btn-primary btn-preview"
              >
                {previewLoading ? (
              ...
              <div className="btn-group">
                <button
                  disabled={
                    disabled || exportLoading || displayType === "heatmap"
                  }
                  title={
                    displayType === "heatmap"
                      ? "Export is only available for the 'Contours' display mode"
                      : ""
                  }
                  type="button"
                ...
                <ul className="dropdown-menu  pull-right">
                  <li>
                    <a
                      ...
                      onClick={this.handleExport("GPKG")}
                    >
                  ...
                  <li>
                    <a
                      href="javascript:void(0);"
                      onClick={this.handleExport("ESRI Shapefile")}
                    >
                      <i className="fa fa-file-archive fa-fw"></i> ShapeFile
                      (.SHP)
                    </a>
                  </li>
                </ul>
