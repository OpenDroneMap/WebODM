...
export default class ContoursPanel extends React.Component {
      ...
      this.loadingReq = $.getJSON(`/api/projects/${project}/tasks/${id}/`)
        .done((res) => {
          ...
          if (layers.length > 0) {
            ...
            this.setState({
              permanentError: _(
                "No DSM or DTM is available. To export contours, make sure to process a task with either the --dsm or --dtm option checked."
              ),
            });
          }
        })
            ...
            previewLayer: L.geoJSON(geojson, {
              onEachFeature: (feature, layer) => {
                if (
                  ...
                  layer.bindPopup(
                    `<b>${_("Elevation:")}</b> ${feature.properties.level} ${_(
                      "meters"
                    )}`
                  );
                }
  ...
  render() {
    ...
    const simplifyValues = [
      { label: _("Do not simplify"), value: 0 },
    ...
    if (loading)
      content = (
        <span>
          <i className="fa fa-circle-notch fa-spin"></i> {_("Loading…")}
        </span>
      );
          ...
          <div className="row form-group form-inline">
            ...
            <div className="col-sm-9 ">
              <select
                className="form-control"
                value={simplify}
                onChange={this.handleSelectSimplify}
              >
                {simplifyValues.map((sv) => (
                  <option value={sv.value}>
                    {sv.label} ({sv.value} {_("meter")})
                  </option>
                ))}
                <option value="custom">{_("Custom")}</option>
              </select>
            ...
            <div className="row form-group form-inline">
              ...
              <div className="col-sm-9 ">
                <input
                  type="number"
                  className="form-control custom-interval"
                  value={customSimplify}
                  onChange={this.handleChangeCustomSimplify}
                />
                <span> {_("meter")}</span>
              </div>
            </div>
              ...
              <div className="btn-group">
                ...
                <ul className="dropdown-menu  pull-right">
                  ...
                  <li>
                    ...
                    >
                      <i className="far fa-file-archive fa-fw"></i> ShapeFile
                      (.SHP)
                    </a>
                  </li>
                </ul>
