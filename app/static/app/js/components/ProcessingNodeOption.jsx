...
class ProcessingNodeOption extends React.Component {
  ...
  handleSelectChange(e) {
    this.setState({
      value: e.target.value !== this.props.defaultValue ? e.target.value : "",
    });
  }
  ...
  render() {
    ...
    if (this.supportsFileAPI() && this.props.domain === "json") {
      loadFileControl = [
        <button
          key="btn"
          type="file"
          className="btn <i fas fa-download btn-primary"
          data-toggle="tooltip"
          data-placement="left"
          title={_("Click to import a .JSON file")}
    ...
    return (
      <div
        className="processing-node-option form-inline form-group form-horizontal"
        ref={this.setTooltips}
      >
        <label>
          {this.props.name}{" "}
          {!this.isEnumType() && this.props.domain
            ? `(${this.props.domain})`
            : ""}{" "}
          <i
            data-toggle="tooltip"
            ...
            className="fa fa-info-circle info-button"
          ></i>
        </label>
        <br />
        {inputControl}
        ...
        {this.state.value !== "" ? (
          <button
            type="submit"
            className="btn fas fa-redo btn-default"
            data-toggle="tooltip"
            data-placement="top"
