...
class Task(models.Model):
    ...
    console_output = models.TextField(
        ...
        default="",
        blank=True,
        help_text=_("Console output of the processing node"),
        verbose_name=_("Console Output"),
    )
                ...
                with transaction.atomic():
                    for img in self.imageupload_set.all():
                        ...
                        img.image.name = assets_directory_path(
                            self.id, new_project_id, os.path.basename(img.image.name)
                        )
    ...
    def handle_import(self):
        ...
        if self.import_url and not os.path.exists(zip_path):
            try:
                ...
                #       import tasks
                logger.info(
                    "Importing task assets from {} for {}".format(self.import_url, self)
                )
                        ...
                        if time.time() - last_update >= 2:
                            ...
                            if total_length is not None:
                                Task.objects.filter(pk=self.id).update(
                                    running_progress=(float(downloaded) / total_length)
                                    * 0.9
                                )
                    ...
                    if self.status in [
                        ...
                        if self.status == status_codes.COMPLETED:
                            ...
                            if os.path.exists(assets_dir):
                                logger.info(
                                    "Removing old assets directory: {} for {}".format(
                                        assets_dir, self
                                    )
                                )
